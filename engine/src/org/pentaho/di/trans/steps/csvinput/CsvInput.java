...
public class CsvInput extends BaseStep implements StepInterface {
  ...
  private boolean openNextFile() throws KettleException {
    try {
      ...
      // Open the next one...
      //
      data.fieldsMapping = createFieldMapping( data.filenames[data.filenr], meta, getTransMeta() );
  ...
  FieldsMapping createFieldMapping( String fileName, CsvInputMeta csvInputMeta, VariableSpace variableSpace )
    throws KettleException {
    FieldsMapping mapping = null;
    if ( csvInputMeta.isHeaderPresent() ) {
      String[] fieldNames = readFieldNamesFromFile( fileName, csvInputMeta, variableSpace );
  ...
  String[] readFieldNamesFromFile( String fileName, CsvInputMeta csvInputMeta, VariableSpace variableSpace )
    throws KettleException {
    String delimiter = variableSpace.environmentSubstitute( csvInputMeta.getDelimiter() );
    String enclosure = variableSpace.environmentSubstitute( csvInputMeta.getEnclosure() );
    String realEncoding = variableSpace.environmentSubstitute( csvInputMeta.getEncoding() );
