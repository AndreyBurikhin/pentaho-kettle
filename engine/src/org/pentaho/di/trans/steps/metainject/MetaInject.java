...
public class MetaInject extends BaseStep implements StepInterface {
  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    for ( String targetStep : data.stepInjectionMap.keySet() ) {
      if (!data.stepInjectionMap.containsKey( targetStep )) {
      ...
      if ( target.getStepname().equalsIgnoreCase( targetStep ) ) {
        ...
        if ( rows != null && !rows.isEmpty() ) {
          ...
          for(RowMetaAndData r:rows) {
            values.add( getValue( r, source ) );
          }
          injector.setProperty( targetStepMeta, target.getAttributeKey(), values );
        }
      }
    }    
  }
  ...
  private Object getValue( RowMetaAndData row, SourceStepField source ) throws KettleValueException {
return row.getString( source.getField(), null );
  }
