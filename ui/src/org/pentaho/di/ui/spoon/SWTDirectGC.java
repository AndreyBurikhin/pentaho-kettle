...
public class SWTDirectGC implements GCInterface {
  ...
  public void drawImage( EImage image, int x, int y, float magnification ) {
    Image img =
        getNativeImage( image ).getAsBitmapForSize( gc.getDevice(), Math.round( ecionsize * magnification ),
            Math.round( ecionsize * magnification ) );
    if ( img != null ) {
      Rectangle bounds = img.getBounds();
      gc.drawImage( img, 0, 0, bounds.width, bounds.height, x, y, Math.round( ecionsize * magnification ), Math
          .round( ecionsize * magnification ) );
    }
  }

  public Point getImageBounds( EImage image, float magnification ) {
    Image img = getNativeImage( image ).getAsBitmapForSize( gc.getDevice(), Math.round( ecionsize * magnification ),
        Math.round( ecionsize * magnification ) );
